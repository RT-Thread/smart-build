#!/usr/bin/env python3
"""Build GNU libiconv with the public RT-Smart adaptation."""
...
def main() -> int:
    ...
    run(
        ["patch", "-p1", "--batch", "--forward", "-i", str(source_dir / "01_src_iconv.diff")],
        cwd=source_dir,
    )
    reset_dir(build_dir)
    reset_dir(dest_dir)
    ...
    copy_file(dest_dir / "usr/include/iconv.h", stage_dir / "usr/include/iconv.h", 0o644)
    copy_file(dest_dir / "usr/lib/libiconv.a", stage_dir / "usr/lib/libiconv.a", 0o644)
    copy_file(dest_dir / "usr/lib/libcharset.a", stage_dir / "usr/lib/libcharset.a", 0o644)
    return 0
