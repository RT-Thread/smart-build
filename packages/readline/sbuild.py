#!/usr/bin/env python3
"""Build readline as a static Smart package."""
...
def append_env_flags(env: dict[str, str], key: str, *values: str) -> None:
    env[key] = (env.get(key, "") + " " + " ".join(values)).strip()
...
def main() -> int:
    ...
    ncurses_dir = dependency_root() / "ncurses"
    env = os.environ.copy()
    append_env_flags(env, "CPPFLAGS", "-I" + str(ncurses_dir / "usr/include/ncurses"))
    append_env_flags(env, "LDFLAGS", "-L" + str(ncurses_dir / "usr/lib"))
    ...
    run(["make", "-j" + jobs, "SHLIB_LIBS="], cwd=build_dir, env=env)
    run(["make", "install-static", f"DESTDIR={dest_dir}"], cwd=build_dir, env=env)

    copy_file(dest_dir / "usr/include/readline/readline.h", stage_dir / "usr/include/readline/readline.h", 0o644)
    copy_file(dest_dir / "usr/lib/libreadline.a", stage_dir / "usr/lib/libreadline.a", 0o644)
    return 0
