...
def main() -> int:
    ...
    copy_tree(dest_dir / "usr/include/lzo", stage_dir / "usr/include/lzo")
    libraries = (dest_dir / "usr/lib/liblzo2.a", dest_dir / "usr/lib64/liblzo2.a")
    library = next((path for path in libraries if path.is_file()), None)
    if library is None:
        raise SystemExit("expected file not found: " + " or ".join(str(path) for path in libraries))
    copy_file(library, stage_dir / "usr/lib/liblzo2.a", 0o644)
    return 0
