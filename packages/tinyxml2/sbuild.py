#!/usr/bin/env python3
"""Build TinyXML2 as a shared library for ROS 2 packages."""
...
def main() -> int:
    ...
    copy_tree(dest_dir / "usr/include/tinyxml2.h", stage_dir / "usr/include/tinyxml2.h")
    lib_dir = dest_dir / "usr/lib"
    candidates = [
        path for path in sorted(lib_dir.glob("libtinyxml2.so.*"))
        if path.is_file() and not path.is_symlink()
    ]
    if not candidates:
        raise SystemExit("libtinyxml2.so was not installed")
    copy_tree(candidates[-1], stage_dir / "usr/lib/libtinyxml2.so")
    cmake_dir = dest_dir / "usr/lib/cmake/tinyxml2"
    if cmake_dir.is_dir():
        copy_tree(cmake_dir, stage_dir / "usr/lib/cmake/tinyxml2")
    pkgconfig = dest_dir / "usr/lib/pkgconfig/tinyxml2.pc"
    if pkgconfig.is_file():
        copy_tree(pkgconfig, stage_dir / "usr/lib/pkgconfig/tinyxml2.pc")
    return 0
