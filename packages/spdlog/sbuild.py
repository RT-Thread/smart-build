#!/usr/bin/env python3
"""Build spdlog against the staged fmt library."""
...
def fmt_prefix() -> Path:
    ...
    prefix = staging / "fmt" / "usr"
    if not prefix.is_dir():
        raise SystemExit(f"fmt staging prefix not found: {prefix}")
    return prefix
...
def main() -> int:
    ...
    copy_tree(dest_dir / "usr/include/spdlog", stage_dir / "usr/include/spdlog")
    lib_dir = dest_dir / "usr/lib"
    candidates = [path for path in sorted(lib_dir.glob("libspdlog.so*")) if path.is_file()]
    if not any(not path.is_symlink() for path in candidates):
        raise SystemExit("libspdlog.so was not installed")
    for candidate in candidates:
        copy_tree(candidate, stage_dir / "usr/lib" / candidate.name)
    cmake_dir = dest_dir / "usr/lib/cmake/spdlog"
    if cmake_dir.is_dir():
        copy_tree(cmake_dir, stage_dir / "usr/lib/cmake/spdlog")
    return 0
